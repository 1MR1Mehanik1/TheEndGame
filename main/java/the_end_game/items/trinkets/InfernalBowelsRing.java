...
public class InfernalBowelsRing extends BasicTrinketsItem {
public InfernalBowelsRing(String name, int maxStackSize) { super(name, maxStackSize); }
	
	@Override public TrinketType getTrinketType(ItemStack itemstack) { return TrinketType.RING; }
	
	@Override
	public void addInformation(ItemStack stack, @Nullable World world, List<String> list, ITooltipFlag flag) {
		list.add(I18n.translateToLocalFormatted("trinket.description.ring_infernal_bowels"));
	}
	...
	public void onWornTick(ItemStack stack, EntityLivingBase entityBase) {
		if (entityBase instanceof EntityPlayer) {
			...
			boolean swimming = player.isInsideOfMaterial(Material.LAVA) || player.isInLava();
			if (swimming && player.experienceLevel >= 1 && !player.isCreative()) {
				float speed = 0.06f * 1.2f;
